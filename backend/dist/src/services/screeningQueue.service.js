...
export class ScreeningQueueService {
    queue = [];
    isProcessing = false;
    auditLogPath = path.join(__dirname, '../../uploads/logs/ai_recruitment_audit.log');
    constructor() {
     ...
     */
    queueApplication(applicationId) {
    ...
    async processApplication(applicationId) {
        ...
        if (nextStage === 'INTERVIEW') {
            ...
            await notificationService.sendNotification(job.postedById, 'High-Quality Candidate Alert 🚀', `${candidate.fullName} matched with ${score}% overall match. Automatically moved to INTERVIEW stage.`, 'SYSTEM', `/hr/pipeline`);
            ...
            await notificationService.sendNotification(job.postedById, 'Candidate Screen Ready 📋', `${candidate.fullName} applied and scored ${score}%. Moved to SCREENING.`, 'SYSTEM', `/hr/pipeline`);
        }
...
//# sourceMappingURL=screeningQueue.service.js.map