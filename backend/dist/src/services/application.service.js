...
export class ApplicationService {
    async applyForJob(jobId, candidateId, data) {
        ...
        if (application.candidate.resumeUrl) {
            aiIntegrationService.evaluateApplication(application.id, application.candidate.resumeUrl, application.job.description).catch(err => console.error("AI Evaluation background task failed:", err));
        }
        return application;
    }
    ...
    async updateApplicationStage(id, stage, performedById, note) {
                ...
                if (candidate) {
                    ...
                    if (!profile) {
                        ...
                        try {
                            ...
                            if (tempPassword) {
                                await emailService.sendOnboardingCredentialsEmail(user.email, user.fullName, employeeId, tempPassword);
                            }
                        }
                        catch (err) {
