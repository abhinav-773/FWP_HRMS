...
export const sendChatMessage = async (req: Request, res: Response) => {
  ...
  try {
    // Query Gemini directly using company handbook context
    const prompt = `
    ...
    console.error('[Chat Controller] Gemini failed:', geminiError.message);
...
export const getChatHistory = async (req: Request, res: Response) => {
  // Chat history is now handled locally — no Python microservice
  const { sessionId } = req.params;
  res.json({ session_id: sessionId, messages: [] });
};

export const getUserSessions = async (req: Request, res: Response) => {
  // Sessions are now handled locally — no Python microservice
  res.json({ sessions: [] });
};
