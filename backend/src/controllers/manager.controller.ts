...
export const getTeamDashboard = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const createTeamTask = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = (req as any).user;
    ...
    if (!managerProfile || !managerProfile.department) throw new Error('Manager profile or department missing.');
...
export const getLeaveApprovals = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = (req as any).user;
    const managerProfile = await prisma.employeeProfile.findUnique({ where: { userId: user.userId } });
    if (!managerProfile) throw new Error('Manager profile missing');
...
export const submitPerformanceReview = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = (req as any).user;
...
export const getTeamMembers = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = (req as any).user;
    if (!user.employeeProfileId) throw new Error('Manager profile ID missing in request');
