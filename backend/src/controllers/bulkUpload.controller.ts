...
export const bulkUploadResumes = async (req: Request, res: Response) => {
    ...
    try {
        ...
        fs.appendFileSync('bulk_upload_error.log', new Date().toISOString() + ' - Bulk Upload Route Error: ' + ((error as any).stack || error) + '\n');
        if (!res.headersSent) {
...
async function processResumes(files: Express.Multer.File[], job: any, jobText: string) {
    for (const file of files) {
        try {
            ...
            fs.appendFileSync('bulk_upload_error.log', new Date().toISOString() + ` - Failed to process resume ${file.originalname}: ` + ((error as any).stack || error) + '\n');
        }
    }
}
