...
export class OnboardingService {
  ...
  async createChecklist(employeeId: string, title: string, description?: string, customTasks?: any[]) {
  ...
  async getEmployeeChecklist(employeeId: string) {
  ...
  async updateTaskStatus(taskId: string, status: OnboardingTaskStatus, documentUrl?: string) {
    return await prisma.$transaction(async (tx) => {
      ...
      let newChecklistStatus: OnboardingChecklistStatus = OnboardingChecklistStatus.PENDING;
      if (allCompleted) {
  ...
  // Assign IT Asset
  async assignAsset(employeeId: string, assetName: string, assetType: string, serialNumber?: string) {
    const employee = await prisma.employeeProfile.findUnique({
      where: { id: employeeId },
      include: { user: true }
    });
