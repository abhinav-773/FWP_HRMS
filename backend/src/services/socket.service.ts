...
export const setupSocketService = (io: Server) => {
  ...
  io.use(async (socket: Socket, next) => {
    ...
    if (meetingUrl) {
      ...
      (socket as any).isCandidate = true;
      (socket as any).meetingUrl = meetingUrl;
      return next();
    }
  ...
  io.on('connection', (socket: Socket) => {
    ...
    const isCandidate = (socket as any).isCandidate;
  ...
  eventBus.on('task:completed', ({ managerId, task }) => {
    if (managerId) io.to(`user_${managerId}`).emit('TASK_COMPLETED', task);
  });
  eventBus.on('leave:requested', ({ managerId, request }) => {
    if (managerId) io.to(`user_${managerId}`).emit('LEAVE_REQUESTED', request);
  });
