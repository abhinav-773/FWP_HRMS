...
// ─────────────────────────────────────────────────────────
// Action Registry — Maps intent IDs to executable functions
// ─────────────────────────────────────────────────────────
...
async function hrShowTopCandidates(params: any): Promise<ActionResult> {
  ...
  if (skill) {
    ...
    filtered = applications.filter(app =>
      app.candidate.skills.some((s: string) => s.toLowerCase().includes(skillLower)) ||
...
async function hrGenerateJD(params: any): Promise<ActionResult> {
...
Make it enterprise-grade, compelling, and specific. Do NOT wrap in markdown code fences.`;
  ...
  try {
    ...
    let md = `## ${jd.title}\n\n`;
    md += `${jd.about}\n\n`;
    md += `### Responsibilities\n${(jd.responsibilities || []).map((r: string) => `- ${r}`).join('\n')}\n\n`;
    md += `### Requirements\n${(jd.requirements || []).map((r: string) => `- ${r}`).join('\n')}\n\n`;
    md += `### Nice to Have\n${(jd.niceToHave || []).map((r: string) => `- ${r}`).join('\n')}\n\n`;
    md += `### Benefits\n${(jd.benefits || []).map((r: string) => `- ${r}`).join('\n')}`;
...
async function hrMoveShortlistedToInterview(_params: any): Promise<ActionResult> {
...
async function hrSummarizeATS(_params: any): Promise<ActionResult> {
...
async function mgrShowLowProductivity(params: any): Promise<ActionResult> {
  ...
  const attendanceMap = new Map(
    attendanceAgg.map(a => [a.employeeId, { days: a._count.id, avgHours: a._avg.workHours || 0 }])
  );
...
async function mgrWeeklyPerformanceReport(params: any): Promise<ActionResult> {
...
async function empShowPendingTasks(params: any): Promise<ActionResult> {
  const { userId } = params;
...
async function empSummarizePerformance(params: any): Promise<ActionResult> {
  const { userId } = params;
...
type ActionId = keyof typeof ACTION_REGISTRY;
...
export class CopilotActionEngine {
  async processMessage(
    ...
    if (
      ...
      const actionDef = ACTION_REGISTRY[intent.action as ActionId];
    ...
    const conversationalResponse = await this.generateConversationalResponse(
      userId, role, fullName, message, context
    );
  ...
  private async detectIntent(message: string): Promise<{ action: string; params: any; confidence: number }> {
    try {
