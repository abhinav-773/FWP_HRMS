...
const activeInterviews = new Map<string, {
    candidateSocketId: string | null;
    recruiterSocketIds: Set<string>;
    audioStream: fs.WriteStream | null;
}>();

export const setupInterviewGateway = (io: Server, socket: Socket) => {
    // 1. Candidate Joining the Interview Room
    socket.on('join_interview_room', async (meetingUrl: string) => {
    ...
    socket.on('interview_media_chunk', (data: { interviewId: string, chunk: ArrayBuffer }) => {
    ...
    socket.on('request_next_question', async (data: { interviewId: string }) => {
