...
const activeBreaks = new Map<string, Date>();

export class AttendanceService {
  async clockIn(userId: string, location?: string, workFromHome: boolean = false) {
  ...
  async toggleBreak(userId: string) {
    const profile = await employeeBootstrapService.ensureEmployeeProfile(userId);
    ...
    if (!attendance) throw new Error('You must clock in first before taking a break');
  ...
  async getBreakStatus(userId: string) {
    const profile = await employeeBootstrapService.ensureEmployeeProfile(userId);
    return { onBreak: activeBreaks.has(profile.id) };
  }

  async getBurnoutCheck(userId: string) {
    const profile = await employeeBootstrapService.ensureEmployeeProfile(userId);
